...
//					POINT_CAMERA_AT_POINT -1392.5842 491.3141 19.5280 JUMP_CUT
	...
	IF flagDisplayedFlyingHelp = FALSE
		IF IS_CHAR_IN_MODEL scplayer HYDRA
			PRINT_HELP_FOREVER MAN2_70  // Use right analog stick to control the attitude of the thruster nozzles between down (for take off and landing) and back (for forward thrust).
			...
			AND timerHelpText = 0
				...
				IF IS_CHAR_IN_CAR scplayer carPlayerPlane
					...
					IF flagTempFlag = TRUE
						...
						CLEAR_HELP
						PRINT_HELP_FOREVER HYDHLP1  // Moving the right analog stick down for a few seconds will position the thrusters for hover.
						flagDisplayedSpyShipHelp = TRUE
						timerHelpText = m_mission_timer + 10000
					ENDIF
				ENDIF
